...
class Graph
{
    public static function verticalBar($values, $height, $title = '', $description = '')
    {
        ...
        foreach ($values as $key => $value) {
            ...
            $html .= '<a style="display: block; position: relative; margin: 0 10%; background-color: #aaa; height: ' .
                $p . '%; top: ' . (100 - $p) . '%" title="' . $title . '">';
            $html .= '</a>';
            $html .= '</div>';
            $html .= '</div>';
        }
    ...
    public static function horizontalBar($values, $height, $title = '')
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            if ($i % 2 == 0) {
                $html .= '<div style="position: absolute; left: ' . ($i * 10) .
                    '%; height: 100%; text-align: left; border-left: 1px solid #aaa;">';
                $html .= '</div>';
                ...
                $html .= '<div style="position: absolute; left: ' . ($i * 10) .
                    '%; height: 100%; text-align: left; border-left: 1px solid #ccc;' . $extra . '">';
                $html .= '</div>';
            }
        }
        $html .= '</div>';
        ...
        foreach ($values as $key => $value) {
            ...
            $html .= '<a style="display: block; float: left; position: relative; background-color: #aaa; width: ' . $p .
                '%; height: 100%;" title="' . $title . '">';
            $html .= '</a>';
            $html .= '<div style="float: left; position: relative; width: ' . (100 - $p) .
                '%; height: 100%; line-height: ' . $line . 'px; overflow: hidden;">';
